        ...
        {['phone', 'email'].map((type) => (
          <button key={type} type="button" onClick={() => setPasswordLoginType(type)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              passwordLoginType === type ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
            }`}>
  ...
  return (
    <div className="animate-fadeIn">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 mt-2">Sign in to book services on HireLink</p>
      </div>
      ...
      <div className="mt-8">
        <div className="relative">
          ...
          <div className="relative flex justify-center text-sm"><span className="px-4 bg-white text-gray-500">New to HireLink?</span></div>
        </div>
        <div className="mt-6 text-center">
          <Link to="/customer/register" className="btn-secondary w-full py-3">Create Customer Account</Link>
        </div>
      </div>

      <div className="mt-6 text-center">
        <Link to="/provider/login" className="text-sm text-emerald-600 font-medium hover:underline">
